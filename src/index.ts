...
export const test = () => {
  ...
  startGame(testChannel); // Should send error message
  addPlayer(testChannel, `1`); // Should send error message
  removePlayer(testChannel, `1`); // Should send error message
  readyPlayer(testChannel, `1`); // Should send error message
  mapVote(testChannel, `invalid`, testMap1); // Should send error message
  stopGame(testChannel); // Should send error message

  setChannelGameMode(testChannel, GameMode.Sixes);
  ...
  addPlayer(testChannel, `1`); // Should start game
  stopGame(testChannel);
  stopGame(testChannel); // Should send error message

  startGame(testChannel);
  ...
  for (let x = 0; x < 11; x++) {
    addPlayer(testChannel, `${x + 1}`);
  }
  for (let x = 0; x < 11; x++) {
    removePlayer(testChannel, `${x + 1}`);
  }
  ...
  const s = store.getState();
  const game = s.games[testChannel];
  ...
  setTimeout(() => {
    ...
    // Add last player
    addPlayer(testChannel, `12`);

    startGame(testChannel); // Should send error message
    addPlayer(testChannel, `12`); // Should send error message
    mapVote(testChannel, `invalid`, testMap1); // Should send error message
    stopGame(testChannel); // Should send error message

    readyPlayer(testChannel, `12`); // Should work

    removePlayer(testChannel, `12`); // Should work - take back
    if (store.getState().games[testChannel].state !== GameState.AddRemove) {
